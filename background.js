  ...
  if (request.action === "disable_for_website") {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      chrome.storage.local.get(["blocklist"], function (result) {
        disabledWebsites = result.blocklist || [];
        ...
        if (domainIndex !== -1) {
          ...
          disabledWebsites.splice(domainIndex, 1);
        ...
        chrome.storage.local.set({ blocklist: disabledWebsites }, function () {
...
// Function to be used by the content script to check whether the current website is disabled or the extension is paused
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.action === "check_website_status") {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      let url = new URL(tabs[0].url);
      sendResponse({
        isDisabled: disabledWebsites.includes(url.hostname),
